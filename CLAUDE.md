...
| Stage 3 | 真值驱动重构（[roadmap](docs/planning/2026-05-30-stage3-truth-driven-refactor.md), [changelog](docs/changelog/2026-05-30-stage3.md)） | ✅ **完成** (2026-05-30) — Phase A-D 完成（5/7 任务真正完成，2/7 推迟）。Phase A: 武器 ani 入 shard + TS truth SOT + 砍 default.json + ActorFactory PVF 真值 + DamageFormula 修正 + 激进改名 + PoC truth 测试。Phase B: 切 beamsword/beamswdc + 14 动画 atkBox 覆盖率报告。Phase C: reaction 公式反推文档骨架 + D9=B 降级路径。Phase D: tests/legacy/ 创建 + truth 测试集成 + bundle visualizer (dist/stats.html 190KB, 主包 1.7MB) + baseline 数据恢复。**核心成果：玩家不再一击秒杀**（attack1 → grunt 23 dmg, 160→137 HP）。80/82 静态测试通过，2 个 fail 是 pre-existing flaky test。**质检教训**：T-D.1/T-D.4 初次只写文档声称完成，质检后真正完成|
