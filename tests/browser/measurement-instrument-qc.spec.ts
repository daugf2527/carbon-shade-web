 ...
 * helper 提炼正确 → A/D 数据必须恒定到当前运行时真值。
 *
...
test.describe.serial("测量仪器 QC", () => {
  ...
  test("A. ground-truth 手动步进 attack1 时长 (期望零抖动)", async ({ page }) => {
    ...
    // 当前场景真值：attack1 命中后 hit-stop 计入 action 生命周期，总计 8 tick。
    expect(s.min).toBe(8);
  });
  ...
  test("D. reset+战斗确定性 (查 serial 污染根)", async ({ page }) => {
    ...
    // 若 reset 干净 + 手动步进确定 → 每次伤害恒定到当前场景真值。
    ...
    // reset 确定性 + 手动步进 → 伤害必须恒定，且等于当前场景真值 55（堵"恒 0 假绿"）
    expect(s.stable, `reset 应确定性清场，实测伤害 ${s.min}~${s.max}`).toBe(true);
    expect(s.min, `第二拳应正常结算 55 伤害，实测 ${s.min}`).toBe(55);
  });
});
