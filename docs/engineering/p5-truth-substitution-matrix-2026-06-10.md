...
| `tests/truth/hit-resolution-weapon-timeline.test.ts` | `src/combat/types/DualTimelineAction.ts` | 无 | 缺失 | 先在 engine 侧建立 weapon timeline flatten / hitbox 提取 truth，摆脱 combat dual timeline type |
...
1. 现在 5 个仍绑 `src/combat/*` 的 truth 里，2 个已经有 engine 层部分替身，3 个没有。
...
- 本文档解决“5 个 truth blocker 里，哪些已经有 engine 替身，哪些没有”。
