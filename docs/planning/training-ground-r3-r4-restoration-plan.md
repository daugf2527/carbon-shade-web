...
> **状态：B/C/D 段基本完成 (~90%) | A 段部分完成 (~60%) | E 段待实现 ❌** (2026-04-29 核查)
>
> B 段（Z 轴移动）、C 段（调试快捷键）、D 段（音效）已基本落地，功能可运行。A 段（视觉升级）功能可用但架构偏离计划——全部内联在 CombatScene.ts 一个文件里，缺少独立的类抽象和对象池。E 段（打击感/物理）完全未动。下文逐项标注实际完成度。
>
