...
> ✅ **2026-05-29 更新**：Phase 0 **全部完成** (commit 95eb84c)。T0.1 flatc 工具链就绪，T0.3 ShardLoader 实现，T0.4 GameLoop 60Hz 验证通过（6 项自动化测试 + 浏览器 demo）。详见 `docs/verification/gameloop-60hz-verification.md`。

| 任务 | 内容 | 验收 | 估时 | 状态 |
|------|------|------|------|------|
| **T0.1** flatc CLI | 从 GitHub Release 下载 flatc 二进制到 `tools/` | `flatc --version` 输出正常 | 30min | ✅ 完成 (commit 95eb84c) |
| **T0.2** LFS push | `git lfs push origin --all`（含 Script.pvf + baseline.db） | Termux 侧 `git lfs pull` 成功 | 30min | ✅ 完成 (commit 938fe29) |
| **T0.3** runtime shard loader | 写 `src/engine/loader/ShardLoader.ts`，从 dist/data/ 异步加载 JSON shards 给 Phase 3 用（FlatBuffers 就绪前 fallback） | `loadShard("players/swordman")` 返回 typed data | 2h | ✅ 完成 (commit 95eb84c) |
| **T0.4** GameLoop 骨架 | 写 `src/engine/core/GameLoop.ts` — 固定步长累加器 + 渲染插值 | 60Hz tick loop 跑起来（空循环） | 2h | ✅ 完成 (commit 95eb84c, 验证: scripts/verify-gameloop.mjs + demo-gameloop.html) |
