...
- Frenzy has real HP drain that pauses during HitStop, swaps basic attack routing to Frenzy basics, reduces supported Berserker cooldowns by 20%, and restores HP after killing a bleeding target.
...
2. Frenzy is still simplified. Real DFO Frenzy includes HP activation/upkeep cost, hit rate, hit recovery, Berserker skill attack increase, selected cooldown reduction, dual-wield basic/dash/jump conversion, and HP restoration from killing bleeding enemies. Current code covers upkeep drain, basic routing, DashAttack/JumpAttack entry, cooldown reduction hooks, and bleeding-kill HP restoration.
...
2. Expand Frenzy into modifiers: activation cost, periodic HP cost, hit recovery, and skill attack multiplier.
