...
export class CombatEventBus {
  ...
  on(type: CombatEventType, handler: CombatEventHandler): void {
  ...
  emit<T>(type: CombatEventType, priority: CombatEventPriority, tick: number, payload: T, meta: Partial<CombatEvent> = {}): CombatEvent<T> {
    const event: CombatEvent<T> = {
      id: nextId("evt"),
      type,
      priority,
      status: "queued",
      tick,
      correlationId: meta.correlationId ?? meta.causationId ?? nextId("corr"),
      parentEventId: meta.parentEventId,
      causationId: meta.causationId,
      sourceActorId: meta.sourceActorId,
      targetActorId: meta.targetActorId,
      payload,
      tags: meta.tags ?? [],
      order: ++this.order,
    };
    if (this.flushing && (priority >= CombatEventPriority.Interrupt || priority === CombatEventPriority.Death)) {
  ...
  flush(): CombatEvent[] {
    ...
    while (this.currentQueue.length > 0) {
      ...
      if (reason) {
        ...
        event.tags.push(`blocked:${reason}`);
        this.archive.push(event);
        flushed.push(event);
        continue;
      }
      event.status = "dispatching";
      for (const handler of this.handlers.get(event.type) ?? []) handler(event, this);
  ...
  drainAll(): CombatEvent[] {
    const drained: CombatEvent[] = [];
    while (this.currentQueue.length > 0 || this.nextQueue.length > 0) {
      drained.push(...this.flush());
    }
    return drained;
  }
